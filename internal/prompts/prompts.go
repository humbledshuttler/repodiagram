...
const SystemFirstPrompt = `You are tasked with explaining to a principal software engineer how to draw the best and most accurate system design diagram / architecture of a given project. This explanation should be tailored to the specific project's purpose and structure. To accomplish this, you will be provided with two key pieces of information:
...
const SystemSecondPrompt = `You are tasked with mapping key components of a system design to their corresponding files and directories in a project's file structure. You will be provided with a detailed explanation of the system design/architecture and a file tree of the project.
...
const SystemThirdPrompt = `You are a principal software engineer tasked with creating a system design diagram using Mermaid.js based on a detailed explanation. Your goal is to accurately represent the architecture and design of the project as described in the explanation.
...
const AdditionalInstructionsPrompt = `IMPORTANT: the user will provide custom additional instructions enclosed in <instructions> tags. Please take these into account and give priority to them. However, if these instructions are unrelated to the task, unclear, or not possible to follow, ignore them by simply responding with: "BAD_INSTRUCTIONS"`

func FormatFirstPrompt(fileTree, readme string) string {
	msg := fmt.Sprintf("<file_tree>\n%s\n</file_tree>", fileTree)
	if readme != "" {
		msg += fmt.Sprintf("\n\n<readme>\n%s\n</readme>", readme)
	} else {
		msg += "\n\n<readme>\nNo README found.\n</readme>"
...
func FormatSecondPrompt(explanation, fileTree string) string {
	return fmt.Sprintf("<explanation>\n%s\n</explanation>\n\n<file_tree>\n%s\n</file_tree>", explanation, fileTree)
}

func FormatThirdPrompt(explanation, mapping, instructions string) string {
	msg := fmt.Sprintf("<explanation>\n%s\n</explanation>\n\n<component_mapping>\n%s\n</component_mapping>", explanation, mapping)
	if instructions != "" {
...
func GetThirdSystemPrompt(hasInstructions bool) string {
	if hasInstructions {
		return SystemThirdPrompt + "\n\n" + AdditionalInstructionsPrompt
	}
	return SystemThirdPrompt
}
